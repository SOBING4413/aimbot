...
function InterfaceManager:ImportSettings()
    pcall(function()
        if not DEBUG and getfenv().isfile and getfenv().readfile and getfenv().isfile("UISettings.sobing") and getfenv().readfile("UISettings.sobing") then
            for Key, Value in next, HttpService:JSONDecode(getfenv().readfile("UISettings.sobing")) do
...
function InterfaceManager:ExportSettings()
    pcall(function()
        if not DEBUG and getfenv().isfile and getfenv().readfile and getfenv().writefile then
            getfenv().writefile("UISettings.sobing", HttpService:JSONEncode(UISettings))
        end
    end)
end
...
pcall(function()
    if not DEBUG and getfenv().isfile and getfenv().readfile and getfenv().isfile(string.format("%s.sobing", game.GameId)) and getfenv().readfile(string.format("%s.sobing", game.GameId)) and UISettings.AutoImport then
        ImportedConfiguration = HttpService:JSONDecode(getfenv().readfile(string.format("%s.sobing", game.GameId)))
...
do
    local Window = Fluent:CreateWindow({
        Title = "Admin Panel",
        SubTitle = #Status > 0 and Status or "by.Sobing4413",
    ...
    if getfenv().isfile and getfenv().readfile and getfenv().writefile and getfenv().delfile then
        ...
        ConfigurationManager:AddButton({
            ...
            Callback = function()
                xpcall(function()
                    if getfenv().isfile(string.format("%s.sobing", game.GameId)) and getfenv().readfile(string.format("%s.sobing", game.GameId)) then
                        local ImportedConfiguration = HttpService:JSONDecode(getfenv().readfile(string.format("%s.sobing", game.GameId)))
                        for Key, Value in next, ImportedConfiguration do
